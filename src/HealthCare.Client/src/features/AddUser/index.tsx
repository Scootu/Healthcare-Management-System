...
const ninRegex = /^\d{6,20}$/; // backend allows up to 20 digits
  ...
  const onSubmit = async (data: userType) => {
    try {
      ...
      if (result) {
        ...
        toast.textContent = "Patient account created successfully! Logging you in...";
        document.body.appendChild(toast);
        
        // Auto-login after successful registration
        try {
          await handleLogin({ nin: String(data.nationalId ?? ""), password: data.password! });
          navigate("/"); // Patient goes to main dashboard
        } catch (loginErr) {
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Controller
            ...
            rules={{
              ...
              pattern: { value: ninRegex, message: "6 to 20 digits" },
            }}
            render={({ field }) => (
              <TextField
                {...field}
